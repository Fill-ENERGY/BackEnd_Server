...
public interface ReviewRepository extends JpaRepository<Review, Long> {
    ...
    Page<Review> findAllByStationIsAndCreatedAtLessThanOrderByCreatedAtDesc(Station station, LocalDateTime createdAt, Pageable pageable);

    @Query(value = "SELECT r1.* FROM review r1 JOIN (SELECT r2.review_id, CONCAT(LPAD(r2.score, 10, '0'), LPAD(r2.review_id, 10, '0')) as cursorValue FROM review r2) as cursorTable ON cursorTable.review_id = r1.review_id WHERE cursorValue < (SELECT CONCAT(LPAD(r2.score, 10, '0'), LPAD(r2.review_id, 10, '0')) FROM review r2 WHERE r2.review_id = :reviewId) AND r1.station_id = :stationId ORDER BY score DESC, review_id DESC  LIMIT :offset", nativeQuery = true)
    List<Review> findAllByOrderByScore(@Param("stationId") Long stationId, @Param("reviewId") Long reviewId, @Param("offset") int offset);

    Page<Review> findAllByStationIsOrderByScoreDescIdDesc(Station station, Pageable pageable);
